...
#include <linux/mutex.h>
  ...
  static DEFINE_MUTEX(verrou); //initialisation du mutex d'une maniere static
                        ...
                        size_t length, loff_t * offset)
{
  ...
  mutex_lock(&verrou); // proteger la section critique avec lock
...
mutex_unlock(&verrou);// liberer la section critique
